...
private:
  ...
  void fillSimHitsInfo(const TrackingParticle& simTrack,
		       vector<float>& result);
...
void McMatchTrackAnalyzer::fillSimHitsInfo(const TrackingParticle& simTrack,
					   vector<float>& result)
{
  // fill sim hits info
  ...
  */
      ...
      // double simhitssize  = double(simTrack.trackPSimHit().size()); // hits from all detectors, in 5.3.X
      double simhitssize  = double(simTrack.numberOfHits()); // hits from all detectors, in 7.X
      
      // double simtrkerhits = double(simTrack.trackPSimHit(DetId::Tracker).size()); // tracker hits, in 5.3.X
      double simtrkerhits = double(simTrack.numberOfTrackerHits()); // tracker hits, in 7.X
      int simpixelhits = 0;
      int simstriphits = 0;
      
      // double simmuonhits  = double(simTrack.trackPSimHit(DetId::Muon).size()); // muon hits, in 5.3.X
      double simmuonhits  = 0; // muon hits
      int simdthits    = 0;
      int simcschits   = 0;
      int simrpchits   = 0;
     ...
     // in 7.X
     for (std::vector<SimTrack>::const_iterator simtrk = simTrack.g4Tracks().begin(); simtrk != simTrack.g4Tracks().end(); simtrk++)
     for (std::vector<PSimHit>::const_iterator simHit = simtrk.g4Tracks().begin(); simHit != simtrk.g4Tracks().end(); simHit++)
	{
     ...
     if (detector == DetId::Muon) simmuonhits++;
  ...
  if(!trkRef.isNull())
    {
      ...
      result.push_back(hp.numberOfHits());
      ...
      result.push_back(hp.numberOfLostHits());
      
      result.push_back(hp.numberOfLostTrackerHits());
      result.push_back(hp.numberOfLostPixelHits());
      result.push_back(hp.numberOfLostStripHits());
  ...
  for (b = in; b != fin; ++b)
	...
	*(p->production_vertex()->particles_in_const_begin()) : 0;
      ...
      if( mother!=0 && !(std::isnan(mother->momentum().perp())) && !std::isnan(abs(mother->pdg_id())) )
	{
	  ...
	  if( parentId != p->pdg_id() && !(parentId<=100 && parentId>=80) ) 
	    {
	  ...
	  if( motherMother!=0 && !(std::isnan(motherMother->momentum().perp()) ) && !(std::isnan(motherMother->pdg_id()) ) )
	    { 
	      ...
	      if( parentId != p->pdg_id()  && !(parentId<=100 && parentId>=80) ) 
		{
	      ...
	      // 3rd mother:
	      const HepMC::GenParticle *motherMotherMother = motherMother->production_vertex() &&
		motherMother->production_vertex()->particles_in_const_begin() != motherMother->production_vertex()->particles_in_const_end() ?
		*(motherMother->production_vertex()->particles_in_const_begin()) : 0 ;
	      
	      if( motherMotherMother!=0 && !(std::isnan(motherMotherMother->momentum().perp()) ) && !(std::isnan(motherMotherMother->pdg_id()) ) )
		{ 
		  parentId = motherMotherMother->pdg_id();
		  if( parentId != p->pdg_id()  && !(parentId<=100 && parentId>=80) ) 
		    {
		      edm::LogInfo("McMatchTrackAnalyzer::getSimParentId")<<"Id_parent = "<<parentId;
		      return parentId;
		  ...
		  // 4ht mother
		  const HepMC::GenParticle *motherMotherMotherMother = motherMotherMother->production_vertex() &&
		    motherMotherMother->production_vertex()->particles_in_const_begin() != motherMotherMother->production_vertex()->particles_in_const_end() ?
		    *(motherMother->production_vertex()->particles_in_const_begin()) : 0 ;
		  
		  if( motherMotherMotherMother!=0 && !(std::isnan(motherMotherMotherMother->momentum().perp()) ) && !(std::isnan(motherMotherMotherMother->pdg_id()) ) )
		    { 
		      parentId = motherMotherMotherMother->pdg_id();
		      if( parentId != p->pdg_id()  && !(parentId<=100 && parentId>=80) ) 
			{
			  edm::LogInfo("McMatchTrackAnalyzer::getSimParentId")<<"Id_parent = "<<parentId;
			  return parentId;
