 ...
 // Get muon tracks only
  ...
  if(dosim2reco)
      ...
      if(matchtrackertracks)
	{
	  ...
	  reco::SimToRecoCollection trkSim2Reco = theAssociatorByHits->associateSimToReco(trackCollection,simCollection,&ev,0);
  ...
  if(doreco2sim)
      ...
      if(matchtrackertracks)
	{
	  ...
	  reco::RecoToSimCollection trkReco2Sim = theAssociatorByHits->associateRecoToSim(trackCollection,simCollection,&ev,0);
...
Int_t McMatchMuonAnalyzer::getSimParentId(const TrackingParticleRef& match)
{
  ...
  for (TrackingParticle::genp_iterator b = match->genParticle_begin(); b != match->genParticle_end(); ++b)
    {
      const HepMC::GenParticle *mother = 0;//(*b)->production_vertex() && 
...
//___________________________________________________________________________
Int_t McMatchMuonAnalyzer::getNumberOfPixelHits(const TrackingParticle& simTrack_)
{
  ...
  edm::LogWarning("MCMatchAnalyzer::getNumberOfPixelHits()")<<"num tracks " << simTrack->g4Tracks().size();
